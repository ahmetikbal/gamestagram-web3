...
class GameViewModel extends ChangeNotifier {
  ...
  void shuffleCurrentGames() {
    if (_games.isNotEmpty) {
      _games.shuffle();
      AppLogger.info(
        'Shuffled ${_games.length} games for new random order',
        'GameViewModel',
      );
      notifyListeners();
    }
  ...
  Future<void> loadInitialGames({int? count}) async {
    ...
    try {
      ...
      final initialGames = await _gameService.fetchGamesFast(
        count: count ?? _initialLoadCount,
      );
      AppLogger.debug(
        'Got ${initialGames.length} random games from service',
        'GameViewModel',
      );

      if (initialGames.isEmpty) {
        AppLogger.warning(
          'No games available from Firebase. Attempting migration...',
          'GameViewModel',
        );

        // Try to migrate from JSON to Firebase
        final migrationSuccess =
            await _gameService.migrateGamesFromJsonToFirebase();
        if (migrationSuccess) {
          AppLogger.info(
            'Migration successful! Reloading games...',
            'GameViewModel',
          );
          ...
          if (migratedGames.isNotEmpty) {
            AppLogger.info(
              'Successfully loaded ${migratedGames.length} games after migration',
              'GameViewModel',
            );
            // Continue with normal processing using migrated games
            final processedGames = await _processGames(
              migratedGames,
        ...
        AppLogger.error(
          'No games available and migration failed',
          'GameViewModel',
        );
        _games = [];
        return;
  ...
  Future<List<GameModel>> _processGames(
    ...
    for (final game in gamesToProcess) {
      try {
        ...
        game.commentCount =
            game.commentCount; // Keep existing count from Firebase
  ...
  /// Fetches all saved games directly from Firestore, regardless of which games are currently loaded
  Future<void> fetchSavedGames(String userId) async {
    try {
      // Reset saved games list
      _savedGames = [];

      // 1. Get saved game IDs from Firebase
      final savedGameIds = await _socialService.getSavedGameIds(userId);

      if (savedGameIds.isEmpty) {
        notifyListeners();
        AppLogger.info(
          'No saved games found for user $userId',
          'GameViewModel',
        );
        return;
      }
      ...
      AppLogger.info(
        'Found ${savedGameIds.length} saved game IDs for user $userId',
        'GameViewModel',
      );

      // 2. Fetch full game data for each saved game ID directly from Firestore
      List<GameModel> fullSavedGames = [];

      // Use batching for better performance (Firestore has a limit of 10 items for "whereIn")
      for (int i = 0; i < savedGameIds.length; i += 10) {
        final int end =
            (i + 10 < savedGameIds.length) ? i + 10 : savedGameIds.length;
        final batch = savedGameIds.sublist(i, end);
        ...
        for (var doc in gamesQuery.docs) {
          ...
          final game = GameModel(
            ...
            isLikedByCurrentUser: false, // Will be updated later if needed
            isSavedByCurrentUser: true, // These are definitively saved
          );
          fullSavedGames.add(game);
        }
      }

      // 3. Update the saved games list with complete data
      _savedGames = fullSavedGames;

      // 4. Also update any existing games in _games list to reflect saved status
      for (var game in _games) {
        game.isSavedByCurrentUser = savedGameIds.contains(game.id);
      }

      AppLogger.info(
        'Successfully loaded ${_savedGames.length} saved games from Firestore',
        'GameViewModel',
      );
      notifyListeners();
    } catch (e) {
      AppLogger.error(
        'Error fetching saved games from Firestore: $e',
        'GameViewModel',
      );
    }
  }
  ...
  Future<void> loadSavedGames(String userId) async {
    try {
      ...
      _savedGames =
          _games.where((game) => savedGameIds.contains(game.id)).toList();
      ...
      notifyListeners();
      AppLogger.info(
        'Loaded ${_savedGames.length} saved games for user $userId',
        'GameViewModel',
      );
    } catch (e) {
  ...
  Future<void> loadLikedGamesStatus(String userId) async {
    try {
      ...
      AppLogger.info(
        'Updated liked status for ${_games.length} games for user $userId',
        'GameViewModel',
      );
    } catch (e) {
  ...
  Future<void> loadUserData(String userId) async {
    try {
      AppLogger.info(
        'Loading all user data from Firebase for user $userId',
        'GameViewModel',
      );
      ...
      AppLogger.info(
        'Successfully loaded all user data from Firebase',
        'GameViewModel',
      );
    } catch (e) {
      AppLogger.error(
