...
class ImageValidator {
  ...
  static const Duration _networkTimeout = Duration(seconds: 3); // Faster timeout
    ...
    try {
      ...
      final response = await http.get(
        ...
        headers: {
          ...
          'Range': 'bytes=0-1023', // Only download first 1KB to check if it's valid
        },
      ).timeout(_networkTimeout);
      
      // Check if response is successful and content type is an image
      if (response.statusCode == 200 || response.statusCode == 206) { // 206 for partial content
